...
#
# $Id: radiod.py,v 1.1 2024/12/19 15:30:02 bob Exp $
#
# Author : Bob Rathbone
...
def interrupt():
    ...
    if interrupt:
        ...
        if event_type > 0:
            log.message(
                "radiod.py Event detected " + event_name + " type " + str(event_type),
                log.DEBUG,
            )
        else:
            interrupt = False
...
def handleEvent(event, display, radio, menu):
    ...
    # to menu pressed (if no separate menu switch eg. Pirate Audio ST7789TFT)
        ...
        event_type == event.UP_SWITCH or event_type == event.DOWN_SWITCH
    ...
    if menu_mode == menu.MENU_SLEEP:
        ...
        event_type == event.LOAD_RADIO
        or event_type == event.LOAD_MEDIA
...
def handleRadioEvent(event, display, radio, menu):
    ...
    if event_type == event.VOLUME_UP:
        ...
        if radio.muted():
            ...
            log.message("Mute switch, speech " + str(radio.config.speech), log.DEBUG)
...
def handleUdpEvent(event, display, radio, message):
    ...
    # Version 1.8 onwards of the web interface
        ...
        valid_event = False
        log.message("radio.handleUdpEvent invalid event:", str(event_type), log.ERROR)
...
def changeOption(event, display, radio, menu):
    ...
    if option_index == menu.OPTION_ALARM:
        ...
        option_index == menu.OPTION_ALARMSETHOURS
        or option_index == menu.OPTION_ALARMSETMINS
    ):
        if event_type == event.RIGHT_SWITCH:
