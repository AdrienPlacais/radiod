...
# $Id: get_shoutcast.py,v 1.1 2024/12/19 15:29:59 bob Exp $
#
# Author : Bob Rathbone
...
def getStationUrl(pls_url,m3u_playlist):
    ...
    print(pls_url)
    try:
        response = urllib.request.urlopen(pls_url,timeout=5)
        pls = response.read()
    except Exception as e:
        print(str(e))
        return ''
...
def copyPlaylist(m3u_playlist,count,install):
    global station_count
    print("Processed " + str(count) + " records from Shoutcast data" )
    print("Created " + str(station_count) + " records in ",m3u_playlist)
    choice = ''
