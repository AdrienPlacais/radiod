...
# $Id: configure_ir_remote.sh,v 1.3 2024/12/20 09:31:21 bob Exp $
#
# Author : Bob Rathbone
...
DT_OVERLAY=""
REMOTE_LED=0
PROTOCOLS="rc-5,rc-5-sz,jvc,sony,nec,sanyo,mce_kbd,rc-6,sharp,xmpir-keytable"
...
do
    ...
    "1" "Default GPIO 11 (pin 23) for 26-pin GPIO headers or HiFiBerry DAC" \
    ...
    if [[ ${ans} == '1' ]]; then
        ...
        DESC="All other designs using DAC sound cards, GPIO 16 (pin 36)"
        REMOTE_LED=16   
...
echo "Using kernel event device ${IR_DEV}" | tee -a ${LOG}
...
echo "    sudo ir-keytable -s ${IR_DEV} -t -p ${PROTOCOLS}" |  tee -a ${LOG}
echo "" |  tee -a ${LOG}
